  ...
  useEffect(() => {
    async function fetchAlumnos() {
      try {
        ...
        if (!response.ok) {
          throw new Error(
            "HTTP error! estado: ${response.status} - ${response.statusText}"
          );
        }
  ...
  return (
    <div className="container mx-auto p-6 bg-gray-50 min-h-[calc(100vh-68px)]">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-8">
        <h2 className="text-4xl font-bold text-gray-800 mb-4 sm:sm-0">
          Lista de alumnos
        </h2>
        <Link
          href="/alumnos/add"
          className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-5 rounded-lg shadow-md transition-colors duration-200 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-blue-500"
        >
          Añadir alumnos
        </Link>
      </div>
